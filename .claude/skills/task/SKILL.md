...
## 14. 프로젝트 위키 갱신 — **PR 을 Ready 로 올리기 전**

`CLAUDE.md` 의 "프로젝트 위키" 절이 정본이다. **이 diff 가 위키의 어느 페이지를 낡게 만들었는지 대조한다.**
...
- **갱신할 것이 없으면 PR 본문에 "없음"이라고 적는다.** 판단 자체를 건너뛰지 않는다.
...
- **Draft 로 연다.** 아래를 전부 만족할 때만 `gh pr ready` 로 전환한다 (§8.1).
  ...
  - [ ] **프로젝트 위키를 갱신했다 — 또는 갱신할 것이 없음을 본문에 적었다 (14단계)**
