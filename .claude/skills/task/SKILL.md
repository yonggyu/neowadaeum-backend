...
3. 그래도 없으면 **직접 만든다.** 지어내지 말고 **3~6단계를 먼저 수행해 정의를 읽은 뒤** 돌아와 채운다.
...
```bash
wiki search neowadaeum "<작업 주제>"
```

정리된 결론이 있으면 **일반론을 새로 세우지 말고 그 결정을 따르고**, 참조한 노트를 PR 본문에 밝힌다.
**`docs/adr/` 원본이 정본이고 위키는 색인이다 — 충돌하면 레포가 이긴다.**
없으면 그냥 진행한다. 없다는 사실을 보고할 필요는 없다.
...
## 14. 프로젝트 위키 — **사용자가 별도로 지시할 때만 수정**

`CLAUDE.md` 의 "프로젝트 위키" 절이 정본이다. 프로젝트 위키는 저장소 밖의 별도 상태이므로 **작업 내용만 보고 수정 권한을 추론하지 않는다.** 사용자가 현재 작업에서 위키 수정을 명시하지 않았다면 이 단계 전체를 건너뛰고, 위키 파일 편집·`wiki new`·`wiki build`·`wiki publish`를 실행하지 않는다.

사용자가 명시적으로 요청한 경우에만 **이 diff 가 위키의 어느 페이지를 낡게 만들었는지 대조하고** 아래 표를 적용한다.

| 이 PR 이 바꾼 것 | 갱신할 페이지 |
|---|---|
| 작업 완료 · 이슈 개폐 · 남은 결정 | `00-Overview/current-status` — **상단의 기준 커밋·날짜를 함께 고친다** |
...
| 문서↔코드 어긋남을 발견하거나 해소함 | `00-Overview/doc-code-drift` |
| 브랜치 · CI · 테스트 정책 | `70-Development/*` |

```bash
wiki read    neowadaeum 00-Overview/current-status.md   # 고치기 전에 읽는다
# 편집
wiki build   neowadaeum                                 # 링크·Mermaid 검증. 실패하면 고칠 때까지 Ready 로 올리지 않는다
wiki publish neowadaeum "<작업번호> <한 줄>"
```

- **별도 지시가 없으면 갱신 여부를 판단하거나 PR 본문에 사유를 적지 않는다.**
- **`docs/internal/` 원문·세이프티 임계값·블록리스트 항목·시크릿을 옮기지 않는다** (S-11). 조항 번호로만 지목한다.
- 근거는 **파일 또는 ADR/R 조항 단위**로 남긴다. 줄 번호는 쓰지 않는다.
- 컬럼 목록·시그니처·디렉터리 트리를 복사하지 않는다. *왜 그런가*를 쓴다.
- 새 노트를 만들면 **그 폴더 `index.md` 에 링크를 추가한다.**
- **위키는 레포 밖이라 CI 가 검증하지 못한다.** `wiki build` 가 유일한 자동 게이트다.
...
- **Draft 로 연다.** 아래를 전부 만족할 때만 `gh pr ready` 로 전환한다 (§8.1).
  ...
  - [ ] PR 템플릿 전 섹션 작성
  - [ ] 최신 `backend` 반영, 충돌 없음
  - [ ] diff 를 처음부터 끝까지 한 번 읽었다 (13단계)
  - [ ] **사용자의 별도 지시 없이 프로젝트 위키를 수정하지 않았다.** 지시가 있었다면 14단계를 마쳤다
