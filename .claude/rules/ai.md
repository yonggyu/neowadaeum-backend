...
- **`STATE VOCABULARY`도 플랫폼 레이어다** (§13-76). `state_schema`가 선언한 **이름**을 알린다 — 값은 이미 `GAME STATE`에 있다. 작품이 넘기는 것은 목록의 항목이고 문장은 코드가 만든다. 묶음 175이며 **축소 대상이 아니다** — 이름을 감추면 그 조건이 영원히 거짓이 된다.
- **`stateChanges`의 연산자 표기는 `OUTPUT SPEC`이 정본이다** (§13-82). 어휘 레이어는 **이름만** 인쇄한다. 표기 목록을 프롬프트에 손으로 적지 않는다 — `play :: port`의 열거를 읽어 만들고, 같은 열거를 파서가 쓴다. 그 문구가 길어져 `INSTRUCTION`이 225로 늘었고 `STATE VOCABULARY`가 같은 만큼 줄었다.
