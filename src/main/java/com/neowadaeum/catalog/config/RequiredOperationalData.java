 ...
 *
 * <p><b>목록이 코드에 하나여야 하는 이유.</b> 이 값들이 막는 화면은 계속 늘어 왔다 (§13-52).
 * 목록을 문서나 절차서에 두면 화면이 늘어도 따라가지 않는다 — 실제로 그렇게 어긋난 표가 있었다.
 * 늘리는 자리와 검사하는 자리가 같아야 한다. <b>수를 적는 것도 목록을 밖에 두는 것과 같다</b> —
 * 세어 적은 문장은 자리가 늘 때마다 틀리므로 여기서는 세지 않는다 (이슈 #487). 지금 막히는 화면은
 * {@code AiNoticeText#require} 를 부르는 자리 전부다.
 *
 * <p><b>여기 서는 것 — 독립된 값만 든다.</b> 동의 판본 가운데 일부는 여기서 들 값이 아니다
