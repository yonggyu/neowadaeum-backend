 ...
 *
 * <p><b>왜 한 곳에 모으는가.</b> 이 규칙을 쓰는 화면은 계속 늘어 왔다 (#257 · #281 · #289).
 * 판정을 화면마다 복사하면 <b>그중 하나가 언젠가 빈 문자열로 흡수하고</b>, 그 화면만 고지 없이
 * 나간다. 모양이 같아서 합치는 것이 아니라 <b>의미가 같아서</b> 합친다.
 *
 * <p><b>목록은 코드가 든다.</b> 어느 화면이 이 문구를 싣는가는 {@link #require(String)} 을 부르는
 * 자리 전부이며, 문서는 그 자리를 가리킬 뿐 <b>세지 않는다</b> — 세어 적은 문장은 화면이 늘 때마다
 * 틀리고, 틀려도 아무것도 빨개지지 않는다 (이슈 #487).
 *
