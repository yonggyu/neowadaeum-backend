...
public class SessionStarter {
	 ...
	 */
	private UUID createSession(UUID playerRef, UUID storyId) {
		// §13-9 — 작품당 active 세션 1개. DB 의 partial unique index 가 마지막 방어선이고,
		// 이 확인은 사용자에게 409 를 돌려주기 위한 것이다.
		if (this.sessions.existsByPlayerRefAndStoryIdAndStatus(playerRef, storyId, SessionStatus.ACTIVE)) {
		...
		return this.sessions.save(PlaySession.start(playerRef, storyId, storyVersionId,
				this.provider.providerId(), SLICE_MODEL_ID, false, Instant.now(this.clock))).getId();
	}
