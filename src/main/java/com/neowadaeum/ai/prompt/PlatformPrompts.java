...
public final class PlatformPrompts {
	...
	/**
	 * UGC 챕터·엔딩 초안을 만들 때 주는 지시 (R7.14, R7.16, B-52).
	 *
	 * <p><b>§4.3 의 예산 대상이 아니다.</b> 판정·요약 문구와 같다 — 별개의 호출이며 턴 프롬프트에
	 * 실리지 않는다. 초안 결과도 예산 대상이 아니다: 작성자가 고쳐 원고가 되고, 프롬프트로 실리는
	 * 것은 그 원고이지 이 응답이 아니다.
	 *
	 * <p><b>번호를 매기지 말라고 요구한다</b> (R7.14). 모델에게 번호를 맡기면 <b>빠지거나 겹친
	 * 번호</b>가 그대로 원고에 들어온다 — 번호는 서버가 붙인다.
	 *
	 * <p><b>조건을 쓰지 말라고 요구한다</b> (R7.16). AI 가 조건식을 지어내면 작성자는 그것을
	 * 읽지도 고치지도 못하고, 조건 평가기가 그것을 어떻게 읽을지는 아무도 검증하지 않았다.
	 * 조건은 플랫폼이 주는 템플릿에서 고른다.
	 *
	 * <p><b>이 문구가 검수를 대신하지 않는다</b> (R7.15). 초안 결과도 검수 대상이며, 등급 지시는
	 * 재작업 횟수를 줄이려는 것이지 판정이 아니다 (I-12 와 같은 성질).
	 */
	public static final String OUTLINE = """
			한국어 인터랙티브 스토리의 챕터·엔딩 초안을 만듭니다.
			- 15세 이용가. 선정적·폭력적 묘사, 혐오 표현, 실존 인물 묘사 금지.
			- 주어진 세계관에서만 만듭니다. 세계관에 없는 설정을 새로 만들지 않습니다.
			- 번호를 매기지 않습니다. 순서대로만 씁니다.
			- 진행 조건·달성 조건을 쓰지 않습니다. 사람이 읽는 글만 씁니다.
			- title 과 label 은 한 줄짜리 이름입니다. summary 와 epilogue 는 두세 문장입니다.
			JSON 객체 하나만 출력. 설명·코드펜스 금지.
			{"chapters": [{"title": string, "summary": string}],
			 "endings": [{"label": string, "epilogue": string}]}""";
