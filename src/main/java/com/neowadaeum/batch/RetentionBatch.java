 ...
 * 명시한 이유다.
 *
 * <p><b>둘을 한 회차에 부른다.</b> 로그 파기와 세션 만료는 스토어도 소유 모듈도 다르지만
 * <b>같은 질문에 답한다</b> — 지난 것을 언제까지 들고 있는가. 주기를 나누면 "지금 보관 상태가
 * 어떤가"를 두 로그에서 맞춰 봐야 한다.
 *
 * <p><b>한쪽이 실패해도 다른 쪽은 돈다.</b> 스토어가 다르므로 함께 묶을 트랜잭션도 없고,
...
public class RetentionBatch {
	...
	public RetentionBatch(LogRetentionPurge logs, SessionExpiry sessions, StringRedisTemplate redis) {
		this.logs = logs;
	...
	public void run() {
		...
		try {
			...
			log.info("batch.retention.done purgedLogs={} expiredSessions={} tookMs={}", purgedLogs,
					expiredSessions, (System.nanoTime() - startedAt) / 1_000_000);
		}
		finally {
