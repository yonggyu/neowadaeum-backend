 ...
 * 깨진다.
 *
 * <p><b>무엇이 여기 오는가</b> (§13-98, 이슈 #488). {@code VALIDATION_ERROR} 는 <b>요청 자체가
 * 틀린 것</b>을 뜻하며, 판별은 한 문장이다 — <b>같은 요청을 나중에 그대로 다시 보내서 통과할
 * 수 있는가.</b> 통과할 수 있으면 어긋난 것은 요청이 아니라 상태이므로 <b>이 열거가 아니라
 * 상태를 말하는 코드</b>가 답한다. 아래 넷 중 셋이 그 문장에 걸리지만 <b>계약 변경이라
 * 프론트와 순서를 맞춰 후속 이슈에서 옮긴다</b> — 지금의 목록을 새 값의 선례로 읽지 않는다.
 */
