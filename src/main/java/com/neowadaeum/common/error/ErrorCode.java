...
public enum ErrorCode {
	...
	/**
	 * [§13-87] ID 토큰이 <b>서버가 발급한 nonce</b> 를 담고 있지 않다 (이슈 #424).
	 *
	 * <p><b>없든 · 안 맞든 · 만료됐든 · 이미 쓰였든 전부 이 하나다.</b> 어느 쪽인지 알려 주면
	 * 서버가 지금 무엇을 들고 있는지 묻는 창구가 된다 (S-6).
	 *
	 * <p><b>{@link #UNAUTHENTICATED} 와 나눈 이유는 클라이언트가 할 일이 다르기 때문이다.</b>
	 * 그쪽은 <i>"토큰이 없거나 낡았다"</i> 이고 재발급으로 회복되지만, 이쪽은 <b>로그인 왕복을
	 * 처음부터 다시</b> 해야 한다 — nonce 를 새로 받아 새 ID 토큰을 받는 것 말고는 길이 없다.
	 * 합치면 그 분기를 화면이 만들 수 없다.
	 */
	LOGIN_NONCE_INVALID(HttpStatus.UNAUTHORIZED, "로그인을 다시 시도해 주세요."),
	...
	/**
	 * [R8.12] 작성자당 원고 개수 상한 (B-51, §13-91).
	 *
	 * <p><b>{@link #ALREADY_EXISTS}(409) 가 아니다</b> (이슈 #450). 그 문구는 <i>"이미 등록되어
	 * 있어요"</i> 이고, 상한에 닿은 사람은 <b>같은 것을 두 번 만든 것이 아니다</b> — 그 안내를
	 * 그대로 그리면(F-4) 사용자는 무엇을 해야 하는지 알 수 없다.
	 *
	 * <p><b>{@link #STORY_LIMIT_REACHED} 와 같은 403 이다</b> — 둘 다 개수 상한이고, 기다려서
	 * 풀리지 않으며, 지금 만들 수 없다는 같은 말을 한다. 다만 <b>이쪽은 지우면 자리가 난다</b>
	 * — 회복하는 길이 다르므로 코드를 나눈다.
	 *
	 * <p><b>상한 값을 문구에 넣지 않는다</b> (S-6). 넣으면 정책이 코드와 문구 두 곳에 생긴다.
	 */
	DRAFT_LIMIT_REACHED(HttpStatus.FORBIDDEN,
			"만들 수 있는 원고 수를 모두 사용했어요. 쓰지 않는 원고를 지우면 새로 만들 수 있어요."),
	/** 소유자가 아니거나 권한이 없다. */
