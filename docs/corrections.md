...
### 13-17. `[결정 필요]` 세션 시작 시 GameState 초기값이 규정되지 않았다

`engineering-guide` §4.2 는 세션 시작 절차에 *"GameState 초기화 (`state_schema` 기본값)"* 을 넣는다. 그런데 **`state_schema` 에는 기본값을 선언할 자리가 없다** — R4.2 가 규정하는 것은 `min` · `max` · `maxDeltaPerTurn` 뿐이다. 가리키는 대상이 없는 문장이다.

**기본 채택안 (S-5 / S-9-1 에서 적용)**

| 시점 | 동작 |
|---|---|
| 세션 시작 | 수치·플래그·인벤토리가 **비어 있는** GameState. `chapter = 1` · `turn = 0` |
| 첫 변화가 올 때 | 그 수치는 **`min` 을 기준**으로 clamp 규칙을 적용한다 (S-5) |

`0` 을 기준으로 쓰지 않는 이유는 `min` 이 양수인 스키마에서 **범위 밖 값**이 되기 때문이다. 조건 평가에서도 "값이 없다"와 "0 이다"를 구분한다 — 없는 수치는 `lt` 계열에서도 `false` 다 (S-6). 구분하지 않으면 도달하면 안 되는 엔딩이 열린다.

`[결정 필요]` **`state_schema` 에 초기값 필드를 신설할지.** 지금은 원문에 없는 스키마를 발명하지 않기로 하고 위 규칙으로 대신했다. 작품마다 "호감도 30에서 시작" 같은 요구가 생기면 그때 필드가 필요해진다 — **B-08(Catalog 스키마) 복귀 시점**에 정한다. 그 전까지 모든 작품은 빈 상태에서 시작한다.
