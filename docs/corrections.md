...
### 13-16. `[결정 필요]` `ending_def.condition` 과 `is_default` 의 관계가 규정되지 않았다

R2.2 는 *"정확히 1개는 `is_default = true`"*, R7.7 은 *"어떤 조건도 매칭되지 않고 마지막 챕터 `max_turns` 에 도달하면 `is_default` 엔딩으로 종료"* 라고 한다. 그러나 **다음 둘 중 어느 것도 규정되어 있지 않다.**

1. `is_default = true` 인 엔딩이 `condition` 을 가질 수 있는가
2. `is_default = false` 인 엔딩이 `condition` 없이 존재할 수 있는가

2번이 허용되면 그 엔딩은 R7.6 의 `ending_no` 순회에서 **항상 최초 매칭**이 되어 뒤의 엔딩을 전부 가린다. 1번은 폴백이라는 정의(§3.1)와 어긋난다.

**기본 채택안 (S-4 에서 적용)**: **제약을 걸지 않는다.** 원문에 없는 정책을 스키마로 고정하지 않는다. S-4 초안에는 `CHECK (is_default = (condition IS NULL))` 이 있었으나 근거가 원문에 없어 제거했다.

`[결정 필요]` 위 두 가지를 규정할지, 규정한다면 DB CHECK 로 막을지 작성 도구(B-49)에서 막을지. **S-7(Chapter · Ending 엔진) 착수 전에 정해야 한다** — 판정 순회 구현이 이 답에 의존한다. 현재는 시드가 규약을 지키는지를 `CatalogSeedTests.R7_6_default_ending_is_last_in_ending_no_order` 가 확인할 뿐이다.
