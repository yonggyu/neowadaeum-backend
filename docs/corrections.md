...
**판정 호출이 25초 생성 예산의 안인가 밖인가.** §13-19 는 *생성* 예산만 정했고, 판정과 재생성까지 더하면 §6.3 의 서버 28초를 넘을 수 있다. **파이프라인이 판정을 실제로 부르는 시점**(B-30 의 마지막 PR)에서 정한다. #87 이 예산을 값으로 만들어 두었으므로 이제 나눠 줄 수 있다.
