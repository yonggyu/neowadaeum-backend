...
| 지점 | 원문 | 채택 |
|---|---|---|
| 인증 요청·응답 본문 | 경로와 에러 코드만 있고 본문 정의가 없다 | **`{ idToken, birthDate?, consents[]? }` → `{ accessToken, refreshToken, tokenType, expiresIn }`.** `birthDate` · `consents` 는 최초 로그인(가입)에만 필요하다. `playerRef` 를 응답에 넣지 않는다 — 클라이언트가 알 필요가 없다. `[결정 필요]` — B-12 착수 시 확정한다 |
