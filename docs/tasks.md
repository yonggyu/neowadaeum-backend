...
> **S-9 에 추가로 요구되는 것 (S-7 #59/#60 에서 정함).**
>
> **1. AI 제안값이 서버 판정을 우회하지 못한다는 것을 오케스트레이터 레벨에서 검증한다.**
> S-7 은 `ChapterEngine` · `EndingEngine` 이 `chapterAdvanceSuggested` · `endingSuggested` 를
> **받을 파라미터조차 없다**는 것을 시그니처로 고정했다. 그러나 그것은 *엔진이 안 받는다*는 증명이지
> **파이프라인이 그 값을 다른 경로로 반영하지 않는다**는 증명이 아니다 — §4.3 의 6단계에서 파싱된
> 제안값이 9·10단계 판정을 우회해 응답이나 세션 상태에 새어들 수 있다.
> **S-9 통합 테스트**: 두 제안값이 서버 판정과 <b>정반대</b>인 `FixedStoryProvider` 응답을 넣고
> ① 챕터가 넘어가지 않는다 ② 세션이 종료되지 않는다 ③ 응답의 `chapterChanged` · `isEnding` 이
> 서버 판정을 따른다 — 셋을 확인한다 (R7.1, R7.9, I-10).
>
> **2. 챕터별 턴 수 집계.** `ChapterEngine` 은 `turnsInCurrentChapter` 를 입력으로 받는다.
> 근거는 `turn.chapter_no` 이며 **비정규화 카운터 컬럼을 만들지 않는다.**
>
> **3. GameState 초기화.** `state_schema` 에는 기본값을 선언할 자리가 없다(§4.2 가 가리키는 대상이
> 없다). S-5 는 "상태에 없는 수치의 기준값 = `min`"만 정했고, **세션 시작 시의 정식 초기화는
> S-9 에서 `[결정 필요]` 로 올린다.**
