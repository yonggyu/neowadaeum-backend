...
> `CLAUDE.md` 에서 옮겨온 원문이다(v1.2 분할, #35). **분할 시점에 내용을 고치지 않았다.**
> 루트에는 이슈→브랜치→커밋→PR 흐름만 한 줄로 남아 있다.
>
> **이후 변경 (v1.4)** — 외부 프로젝트 위키 갱신을 Draft → Ready 조건에서 뺐다.
> 위키 수정은 사용자가 현재 작업에서 별도로 지시한 경우에만 하며, 정본은 `CLAUDE.md` 의 "프로젝트 위키" 절이다.
>
...
- [ ] **사용자의 별도 지시 없이 프로젝트 위키를 수정하지 않았다.** 명시적 지시가 있었다면 `CLAUDE.md` "프로젝트 위키" 절의 검증·게시 절차를 마쳤다
...
```
feat/*    →   backend    →    dev     →    main    →   staging 검증  →  production release
 (작업)      (작업 공간)    (통합 검증)   (릴리스 후보)      (환경)            (공개)
                                             └─ 브랜치는 여기까지다. 그 오른쪽은 행위다
```

| 브랜치 | 성격 | 보호 |
|---|---|---|
| `main` | **릴리스 후보(release candidate).** 태그(`v0.1.0`)를 붙이는 자리 | 직접 푸시 금지, PR + 승인 1인 이상 |
| `dev` | 통합 검증. backend/frontend 통합 지점 | 직접 푸시 금지, CI 통과 필수 |
| `backend` | 백엔드 작업 공간(long-lived) | 직접 푸시 금지, PR 필수 |
| `feat/*` | 실제 작업 브랜치(short-lived) | — |

#### `main` 머지는 공개가 아니다 (2026-08-29, 이슈 #234)

**`main` 에 머지했다는 것은 "이것으로 나갈 수 있다"이지 "나갔다"가 아니다.** 실제 공개는 그
뒤의 별도 행위다 — 태그를 달고(`v*`), 이미지가 게시되고, 스테이징에서 검증한 뒤에 일어난다.
`Release` 워크플로가 **태그로만** 게시하는 것이 이 분리를 코드로 굳힌 자리다: 브랜치 머지는
어떤 배포도 부르지 않는다.

**둘을 붙여 읽으면 `main` 이 멈춘다.** 실제로 그랬다 — 공개할 준비가 안 됐다는 이유로 릴리스를
올리지 않았고, 그 결과 `.github/workflows/` 가 기본 브랜치에 없어 **nightly 가 한 번도 돌지
않았다** (이슈 #181). GitHub 은 `schedule` 과 `workflow_dispatch` 를 **기본 브랜치의 정의로만**
실행한다. `main` 이 후보를 담는 자리가 되면 nightly 와 `Release` 워크플로가 그 위에서 실제로
검증된다.

**다만 nightly 가 도는 것과 그 결과를 읽는 것은 다른 일이다.** 회차를 쌓는 것은 자동이고 읽는
것은 그렇지 않다 — 승격마다 실측 원장을 대조하는 단계를 §8.7 이 든다 (이슈 #411).

> `main` 이 릴리스 후보라는 것은 **아무거나 올린다는 뜻이 아니다.** §8.7 의 조건은 그대로다.

> 레포가 분리되어 있으므로 이 레포에는 `frontend` 브랜치가 없다. `frontend` 브랜치는 프론트 레포에 동일 구조로 존재하며, 두 레포의 `dev`에서 각각 통합 검증한다.

#### 맞추는 것은 번호가 아니라 조합이다 (2026-09-08, 이슈 #453)

이 자리는 오래 *"릴리스 시점을 맞춘다"* 고만 적혀 있었다. **맞추는 방법이 없었고, 그 사이에
이미 갈라졌다** — 프론트가 `v0.1.0` 을 붙였을 때 백엔드의 태그는 0개였다.

**번호는 두 레포가 각자 센다.** 백엔드는 이 레포의 변경만 보고 세고, 프론트도 그렇게 한다.
번호를 묶으면 **이미지를 둘로 나눈 이유의 절반**을 되돌린다 — 한쪽 오타에 다른 쪽까지
롤백된다.

**맞추는 것은 조합이다.** *어느 프론트와 어느 백엔드가 함께 검증됐는가*는
**`docs/releases.md`** 가 한 행씩 든다. 태그와 **커밋 SHA** 를 함께 적는다 — 이동 가능한 태그
(`latest`)를 만들지 않기로 했으므로 수신 측이 볼 좌표는 SHA 다 (`docs/deployment.md`).

> **백엔드 `v0.1.0` 과 프론트 `v0.1.0` 은 같은 문자열이지 같은 것이 아니다.**
> **태그를 레포 없이 부르지 않는다.** 되돌리는 지시는 태그가 아니라 SHA 로 한다.

근거와 채택하지 않은 안은 `docs/corrections.md` §13-92.
...
| 조건 | `backend`에서 **전체 테스트 + E2E(B-44) 통과**, 그리고 **지연 실측 누적 원장(#345) 대조**(아래) |
...
|---|---|
| 시점 | 마일스톤 완료 + QA 통과 시 |
| 조건 | ① §12.2 마일스톤 DoD 충족 ② PC/Mobile QA 완료 ③ 마이그레이션 롤백 계획 검토 완료 ④ 런북(B-64) 해당 항목 존재 ⑤ **지연 실측 누적 원장(#345) 대조**(아래) |
| 방식 | Merge commit |

`main`은 "지금 배포되어 있거나 **배포 가능한 것**"만 담는다. 기능이 완성됐다는 이유로 올리지 않는다.

#### 승격마다 실측 원장을 대조한다 (2026-09-07, 이슈 #411)

**쌓는 것은 자동이고 읽는 것은 자동이 아니다.** nightly 가 매 회차를 누적 이슈 #345 에 한 줄씩 덧붙이지만, 그 줄을 읽고 판단을 고치는 일에는 주인이 없었다 — 표본 512 회차가 **셋** 쌓이도록 아무도 열지 않았고, 그 사이 `docs/perf/turn-latency.md` 의 살아 있는 결론은 표본 32건짜리로 남아 **스트리밍 판단과 B-48 알람 임계가 그 낡은 값을 인용**하고 있었다. #408 이 고친 것은 증상이고, 원인은 읽는 자리가 없다는 것이다.

그래서 **승격 PR 을 여는 사람이 그 자리에서 원장을 훑는다.**

1. #345 의 누적 표에서 **마지막으로 `docs/perf/turn-latency.md` 에 옮겨 적은 회차 이후**를 읽는다.
2. **판단이 달라진 회차가 있으면 승격 전에 옮겨 적는다.** 승격 PR 에 섞지 말고 별도 PR 로 `backend` 에 넣는다 — 승격은 merge commit 이라 그 안에서 문서를 고치면 어느 작업이 고친 것인지 남지 않는다.
3. 없으면 없다고 PR 본문에 적는다. **읽지 않은 것과 읽고 없는 것은 다르다.**

**무엇이 "판단이 달라진 회차"인지는 여기에 적지 않는다.** 그 조건 셋은 `docs/perf/turn-latency.md` 의 **"다음 측정"** 절이 이미 들고 있다. 같은 조건을 두 곳에 두면 반드시 갈라지고, **갈라진 규칙은 없는 것보다 나쁘다.**

**자동화하지 않는다.** 회차 수로 알리면 우는 조건이 판단과 무관해 곧 무시되는 알람이 되고, 폭 이탈 자동 판정은 **기준 폭을 어디가 갖는가**라는 결정을 하나 더 부른다. 승격은 **이미 사람이 서서 판단하는 자리**다 — 새 알람도 새 임계도 만들지 않고 붙일 수 있는 유일한 자리이기도 하다.

> **대가는 주기가 승격에 묶인다는 것이다.** 승격이 뜸해지면 대조도 뜸해진다. 회차가 더 쌓여 폭의 신뢰구간을 말할 수 있게 되면 자동 판정을 다시 연다 (#411).

#### `main` → 공개 (브랜치가 아니다)

| | |
|---|---|
| 시점 | 후보를 실제로 내보내기로 정할 때 |
| 행위 | **태그**(`v*`) → `Release` 워크플로가 이미지를 게시 → **스테이징 무중단 배포 1회** → **`docs/releases.md` 에 조합 한 행** → production |
| 조건 | ① `docs/deployment.md` §3 의 절차를 지난다. **0번 단계(`ai.notice` 선입력)를 건너뛰지 않는다** ② **번호는 이 레포가 독립적으로 센다**(아래) |

**태그가 릴리스다.** 머지가 아니다 — 언제 나가는지는 사람이 정하고, 그 결정이 태그 하나로
드러나야 롤백할 대상(`docs/deployment.md` §4)도 하나로 남는다.

#### 번호 규약 (2026-09-08, 이슈 #453)

**번호는 이 레포만 보고 센다.** 프론트가 태그를 붙였다는 이유로 백엔드 번호를 올리지 않고,
그 반대도 아니다. **백엔드의 첫 태그는 `v0.1.0`** 이다 — 프론트가 이미 같은 문자열을 썼다는
사실은 백엔드의 번호를 밀지 않는다.

**같은 문자열이 서로 다른 것을 가리킨다.** 그래서 **태그를 레포 없이 부르지 않는다**
(*"백엔드 `v0.1.0`"*), **되돌리는 지시는 SHA 로 한다**, **번호로 조합을 말하지 않는다.**

**조합은 `docs/releases.md` 가 든다.** 스테이징 검증 뒤 production 으로 넘기기 전에 한 행을
적는다 — 한쪽만 새로 나갔어도 적는다(다른 쪽 칸에 직전 행의 값을 옮겨 적는다). 그래야
*"지금 무엇이 함께 도는가"* 가 마지막 행 하나로 읽힌다. **적지 않으면 아무도 모른다** —
그것이 번호를 갈라 세는 대가이고, 이 단계가 그 대가를 무는 자리다. 근거는 §13-92.
