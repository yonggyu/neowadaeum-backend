...
- **신규 `*.yml` 금지.** 예외는 `docker-compose.yml`과 `.github/**/*.yml` 둘뿐. 테스트용 yml을 만들지 않는다 — `@DynamicPropertySource`.
...
| `docs/openapi.yaml` | API 계약 — 런타임 진실의 원천. **아직 없다(B-06). `.gitignore`의 `*.yaml`에 걸리므로 예외가 먼저다 — 이슈 #36** |
